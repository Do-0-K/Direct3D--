...
{
	BoundingBox::CreateFromPoints(bounding_box1, m_nVertices, pxmf3Points, nStride);
}
...
CTexturedBoxMesh::CTexturedBoxMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth, float fxPosition, float fyPosition, float fzPosition) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
	m_nStride = sizeof(CTexturedVertex);
	m_nOffset = 0;
...
CTexturedBoxMesh::~CTexturedBoxMesh()
{
}
